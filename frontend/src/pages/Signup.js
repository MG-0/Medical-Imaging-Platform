...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use the signin function from AuthContext to log the user in immediately
      ...
      setError(
        err.response?.data?.message || "Signup failed. Account may already exist or check your connection.",
      );
    } finally {
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-10">
        <div className="text-center mb-10">
          <div className="bg-blue-600 w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6 rotate-12 shadow-lg">
            <UserPlus className="text-white -rotate-12" size={40} />
          </div>
          <h1 className="text-4xl font-extrabold text-slate-900">
            Create Account
          </h1>
          <p className="text-slate-500 mt-3 text-lg">
            Join the Brain Tumor Detection System
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-center text-slate-600 mt-6 font-medium">
            ...
            <Link to="/signin" className="text-blue-600 hover:text-blue-700 font-bold hover:underline transition-all">
              Sign In
            </Link>
          </p>
        </form>
